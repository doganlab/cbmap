...
class CBMAP(BaseEstimator):
    ...
    >>> rng = RandomState(0)
    >>> n_samples = 1000
    >>> X,  = datasets.make_s_curve(n_samples, random_state=rng)
    
    >>> embedding = CBMAP(n_clusters = 5, clustering_method = "spectral")
    >>> X_transformed = embedding.fit_transform(X)
    >>> X_transformed.shape
    (1000, 2)
    >>> plt.scatter(X_transformed [:,0], X_transformed [:,1], c=embedding.labels)
